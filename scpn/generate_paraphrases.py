...
import torch, time, sys, argparse, os, codecs, h5py, pickle, csv
import numpy as np
...
if __name__ == '__main__':
    ...
    if args.gpu >=0:
        ...
        pp_model = torch.load(args.pp_model, map_location=torch.device('cpu'))
        parse_model = torch.load(args.parse_model, map_location=torch.device('cpu'))

    # load vocab
    pp_vocab, rev_pp_vocab = pickle.load(open(args.vocab, 'rb'))
